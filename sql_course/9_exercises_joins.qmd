...
--SELECT s.spelersnr
--FROM spelers s
--WHERE s.geb_datum < (SELECT s2.geb_datum FROM spelers s2 WHERE s2.naam = 'Permentier' AND s2.voorletters = 'R')
--ORDER BY s.spelersnr;

--SELECT s.spelersnr
--FROM spelers s INNER JOIN spelers s2 ON s2.naam = 'Permentier' AND s2.voorletters = 'R'
--WHERE s.geb_datum < s2.geb_Datum

--=

SELECT s.spelersnr
FROM spelers s, spelers s2 
WHERE s2.naam = 'Permentier' AND s2.voorletters = 'R'
AND s.geb_datum < s2.geb_Datum
:::
...
:::{.notes}
SELECT DISTINCT w.spelersnr, s.naam spelersnaam, t.teamnr, sp.spelersnr aanv_nr, sp.naam aanv_naam, sp.voorletters aanv_voorl
FROM wedstrijden w INNER JOIN spelers s on w.spelersnr = s.spelersnr
INNER JOIN teams t ON w.teamnr = t.teamnr
INNER JOIN spelers sp ON t.spelersnr = sp.spelersnr
WHERE sp.spelersnr <> w.spelersnr
:::
...
:::{.notes}
SELECT b.betalingsnr,b.spelersnr
FROM boetes b 
WHERE b.bedrag IN( SELECT b2.bedrag FROM boetes b2 WHERE b2.spelersnr = 44)
AND b.spelersnr <> 44

SELECT b.betalingsnr, b.spelersnr
FROM boetes b INNER JOIN boetes b2 ON b2.spelersnr = 44
WHERE b.bedrag = b2.bedrag AND b.spelersnr != 44;

SELECT b.betalingsnr, b.spelersnr
FROM boetes b, boetes b2 
WHERE b2.spelersnr = 44 AND b.bedrag = b2.bedrag AND b.spelersnr != 44;
:::
