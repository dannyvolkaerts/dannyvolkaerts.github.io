...
SELECT COUNT(customer_id) customer_amount, state\
FROM customers\
GROUP BY state\
HAVING COUNT(customer_id) = (SELECT COUNT(customer_id) cc FROM customers GROUP BY state ORDER BY cc DESC LIMIT 1)\
...
Which brand has the highest amount of different products in the database?
...
Which brand has the most products in stock of our stores?

:::{.notes}
set search_path TO sales,production; --Postgresql

SELECT COUNT(p.product_id) aantal_producten, b.brand_name as brand 
...
:::{.notes}
SELECT b.brand_name, SUM(s.quantity) as totaal_aantal_product\
FROM products p INNER JOIN brands b ON b.brand_id = p.brand_id\
